...
# The document lives here and nowhere else. hanzoai/cloud is a different
# codebase with its own main, so naming it writes a receipt whose ref cannot
# be fetched. It is also the string a prose sweep reaches for first, which is
# how this line last changed under a comment-only edit.
...
def index(groups, doc):
    ...
    out = ["<!-- GENERATED by publish.py from hanzo.yaml — DO NOT EDIT. "
           "Change the API in hanzo-inc/cloud; group a new capability in "
           ...
           f"`hanzo.yaml` — which is itself generated from hanzo-inc/cloud's own "
    ...
    if a.check:
        ...
        if stale:
            print(f"publish: {', '.join(os.path.basename(f) for f in stale)} is not "
                  ...
                  f"release nobody shipped. Change the API in hanzo-inc/cloud, then run "
