...
### Regenerate the SDKs (canonical)

**openapi-generator, every language, no third-party service and no API key.**
Push a spec change and `regenerate-sdks.yml` validates the merge, then fires a
`spec-update` dispatch at each language repo; that repo's own `generate.yml`
regenerates from this `hanzo.yaml` and opens a PR. Generator pinned to 7.14.0
everywhere, so two languages cannot drift onto different generator behaviour.

| Lang | Repo | Generator |
|------|------|-----------|
| Python | `hanzoai/python-sdk` | `python` (urllib3, pydantic v2) |
| Go | `hanzoai/go-sdk` | `go` |
| TypeScript | `hanzoai/js-sdk` | `typescript-axios` |
| C++ | `hanzoai/cpp-sdk` | `cpp-restsdk` |
| Dart | `hanzoai/dart-sdk` | `dart-dio` |
| Rust | `hanzoai/rust-sdk` | hand-written — openapi-generator emits no Rust |

`LLM.md` is the fuller account. Nothing here calls a hosted generator, so there
is no key to hold and no project to keep pointed at the right branch.
