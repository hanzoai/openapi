...
from — DERIVED by `publish.py` from hanzo-inc/cloud's own emitted `openapi.yaml` at
one pinned release. To change the API, change the code in hanzo-inc/cloud. There is
...
| `.spec-lock` | which hanzo-inc/cloud release it is a projection of — repo, path, ref, sha256. |
...
The first three read a hanzo-inc/cloud checkout. `--served` reads nothing but
...
It reads a hanzo-inc/cloud checkout (`../cloud`, or `--cloud DIR`, or `CLOUD_DIR`)
and writes exactly three files: `hanzo.yaml`, `CAPABILITIES.md`, `.spec-lock`.
Nothing is ever written back to hanzo-inc/cloud.
